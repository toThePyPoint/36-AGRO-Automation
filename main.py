...
if all(is_file_from_today(path) for path in source_files.values()):
    ...
    zkbe1_next_day_df = pd.read_excel(source_files["zkbe1_next_day"], dtype=zkbe1_dtypes)
    zkbe1_next_day_df = zkbe1_next_day_df.rename(columns=zkbe1_columns_names)
    zkbe1_next_day_df['material_number'] = zkbe1_next_day_df['material_number'].astype(str)
    zkbe1_next_day_df = zkbe1_next_day_df[['material_number', 'material_short_text', 'supplier_number', 'supplier_name',
                                           'stock', 'safety_stock', 'planned_delivery_time', 'firmed_issues']]

    zkbe1_today_df = pd.read_excel(source_files["zkbe1_today"], dtype=zkbe1_dtypes)
    zkbe1_today_df = zkbe1_today_df.rename(columns=zkbe1_columns_names)
    zkbe1_today_df['material_number'] = zkbe1_today_df['material_number'].astype(str)
    ...
    merged = merged[['material_number', 'material_short_text', 'supplier_number', 'supplier_name', 'stock',
                     'safety_stock', 'planned_delivery_time', 'buffer', 'firmed_issues_today', 'firmed_issues_next_day',
                     'gap', 'rounding', 'to_trigger', 'Agro_stock', 'quantity_after_issue']]
